 ...
 */
class SharePointAPI { // TODO: to lowercase API (= Api)
	/**
	...
	public function __construct ($spUsername, $spPassword, $spWsdl, $useNtlm = FALSE) {
		...
		try {
			...
			if ($useNtlm === TRUE) {
				// Load include once
				//\TYPO3\CMS\Core\Utility\GeneralUtility::requireOnce(constant('PATH_typo3conf') . 'ext/sharepointconnector/Classes/Client/NTLM_SoapClient.php');

					// Use NTLM authentication client
				$this->soapClient = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('aijko\\SharepointConnector\\Client\\NtlmSoapClient', $this->spWsdl, array_merge($options, array(
				...
				$this->soapClient = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('SoapClient', $this->spWsdl, $options);
			}
